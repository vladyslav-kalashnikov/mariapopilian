...
const categories = ["Всі", "Фотосесії", "Бекстейдж", "Події"];
...
export function PortfolioGallery() {
    ...
    const portfolioItems = [
        { photo: photos["portfolio.gallery.1"], category: "Фотосесії", title: "Editorial Fashion" },
        { photo: photos["portfolio.gallery.2"], category: "Бекстейдж", title: "Runway" },
        { photo: photos["portfolio.gallery.3"], category: "Фотосесії", title: "Portrait" },
        { photo: photos["portfolio.gallery.4"], category: "Події", title: "Gala" },
        { photo: photos["portfolio.gallery.5"], category: "Події", title: "Pageant" },
    ];
        ...
        <section id="portfolio" className="relative overflow-hidden border-t border-white/5 bg-[#050505] py-20 md:py-32">
            ...
            <div className="relative z-10 mx-auto max-w-[1400px] px-5 md:px-10">
                <div className="mb-12 flex flex-col justify-between gap-8 md:mb-20 md:flex-row md:items-end">
                    ...
                    >
                        Вибране <br />
                        <span className="text-[#B39A74] italic">Портфоліо</span>
                    </motion.h2>
                    ...
                    <motion.div
                        ...
                        className="flex gap-6 overflow-x-auto px-5 pb-4 md:mx-0 md:gap-8 md:px-0 md:pb-2 [-ms-overflow-style:none] [-webkit-overflow-scrolling:touch] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
                    >
                    ...
                    {filteredItems.map((item, index) => (
                        ...
                        >
                            <ImageWithFallback
                                ...
                                alt={item.photo.altText || item.title}
                                className="h-full w-full object-cover opacity-80 transition-transform duration-[1.5s] ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:scale-105 group-hover:opacity-100 md:opacity-70"
                            />
            ...
            <AnimatePresence>
                {selectedImage !== null && (
                    <motion.div
                        className="fixed inset-0 z-50 flex items-center justify-center bg-black/98 p-2 backdrop-blur-sm md:p-8"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                    ...
                    >
                        <button
                            className="absolute right-4 top-4 z-50 flex h-12 w-12 items-center justify-center rounded-full bg-white/5 text-white/50 backdrop-blur-md transition-all hover:bg-white hover:text-black md:right-8 md:top-8"
                            onClick={() => setSelectedImage(null)}
                        >
                        ...
                        <motion.div
                            ...
                            className="max-h-[95vh] max-w-full shadow-2xl"
                            onClick={(event) => event.stopPropagation()}
                        >
                            <ImageWithFallback
                                src={filteredItems[selectedImage].photo.imageUrl}
                                alt={filteredItems[selectedImage].photo.altText}
