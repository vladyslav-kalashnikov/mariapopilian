...
export function MediaReels() {
    ...
    const reels = [
        { id: 1, photo: photos["portfolio.reels.1"], title: "Backstage ПЗ", duration: "0:45" },
        { id: 2, photo: photos["portfolio.reels.2"], title: "Runway Highlights", duration: "1:20" },
        { id: 3, photo: photos["portfolio.reels.3"], title: "Vogue Cover Shoot", duration: "0:58" },
    ];
            ...
            <div className="relative z-10 mx-auto max-w-7xl">
                <div className="mb-12 flex flex-col px-5 md:mb-20 md:flex-row md:items-end md:justify-between md:px-10">
                    ...
                    >
                        ...
                        >
                            Motion & <br />
                            <span className="text-[#B39A74] italic">Режисура</span>
                        </h2>
                    </motion.div>
                    ...
                    >
                        <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="group flex items-center gap-3 text-[0.65rem] uppercase tracking-[0.25em] text-white/50 transition-all hover:text-[#B39A74]"
                        >
                            <span>Більше в Instagram</span>
                ...
                <div className="flex snap-x snap-mandatory overflow-x-auto px-5 pb-8 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:grid md:grid-cols-3 md:gap-3 md:px-10 md:pb-0">
                    {reels.map((reel, index) => (
                        <motion.div
                            ...
                            className="group relative mr-4 aspect-[9/16] w-[85vw] min-w-[85vw] shrink-0 snap-center cursor-pointer overflow-hidden bg-[#0a0a0a] md:mr-0 md:w-auto md:min-w-0"
                        >
                            <ImageWithFallback
                                src={reel.photo.imageUrl}
                                alt={reel.photo.altText || reel.title}
                                className="h-full w-full object-cover opacity-70 transition-transform duration-[1.5s] ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:scale-105 group-hover:opacity-90 md:opacity-60"
                            />
                            ...
                            <div className="absolute bottom-0 left-0 right-0 flex translate-y-0 items-end justify-between p-6 transition-transform duration-500 md:translate-y-2 md:p-8 md:group-hover:translate-y-0">
                                ...
                                <span
                                    className="mb-1 shrink-0 text-[#B39A74] md:mb-2"
                                    style={{
                                        fontFamily: "'Inter', sans-serif",
                                        fontSize: "0.6rem",
                                        fontWeight: 500,
                ...
                <div className="mt-4 flex justify-center gap-2 md:hidden">
                    {reels.map((_, index) => (
                        <div key={index} className={`h-1 rounded-full bg-[#B39A74] ${index === 0 ? "w-6 opacity-100" : "w-1.5 opacity-30"}`} />
                    ))}
                </div>
            </div>
        </section>
